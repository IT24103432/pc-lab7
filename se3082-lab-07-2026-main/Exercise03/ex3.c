...
int main(int argc, char *argv[]) {
    ...
    long long *gathered_sums = NULL;

    if (my_rank == 0) {
        full_dataset = (int *)malloc(TOTAL_ELEMENTS * sizeof(int));
        gathered_sums = (long long *)malloc(comm_sz * sizeof(long long));
        if (!full_dataset || !gathered_sums) {
    ...
    printf("Rank %d calculated partial sum = %lld\n", my_rank, my_partial_sum);

    MPI_Gather(&my_partial_sum, 1, MPI_LONG_LONG,
               gathered_sums, 1, MPI_LONG_LONG,
               0, MPI_COMM_WORLD);

    if (my_rank == 0) {
        long long grand_total = 0;
        for (int idx = 0; idx < comm_sz; idx++) {
            grand_total += gathered_sums[idx];
        }
